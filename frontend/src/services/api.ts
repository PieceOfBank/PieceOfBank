...
export const subscriptionPost = (data: Record<string, string>) => {
  return axiosClient.post(`/notification/subscriptions/request`, data);
};

// 2. 보호 관계 수락 (피보호자 → 보호자)
export const subscriptionApproval = (data: Record<string, string>) => {
  return axiosClient.post(`/notification/subscriptions/approval`, { params: data });
};
// 3. 보호 관계 거절 (피보호자 → 보호자)
export const subscriptionRefusal = (data: Record<string, string>) => {
  return axiosClient.post(`/notification/subscriptions/refusal`, { params: data });
};
// 4. 보호 관계 조회
